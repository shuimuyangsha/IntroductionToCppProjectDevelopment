...
bool CDMenu::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if(m_item0->OnLButtonDown(nFlags, point)) {
        // 结束动画
        EndAnimate();
        AfxMessageBox (_T ("设置游戏进入游戏阶段，并设置当前工具为 CShooter0"));
        return true;
    }
    if(m_item1->OnLButtonDown(nFlags, point)) {
        ...
        AfxMessageBox (_T ("设置游戏进入游戏阶段，并设置当前工具为 CShooter1"));
        return true;
    }
    if(m_item2->OnLButtonDown(nFlags, point)) {
        EndAnimate();
        AfxMessageBox(_T("设置游戏进入游戏阶段，并设置当前工具为 CShooter2"));
        return true;
    }
    if(m_item3->OnLButtonDown(nFlags, point)) {
        EndAnimate();
        AfxMessageBox(_T("设置游戏进入游戏阶段，并设置当前工具为 CShooter3"));
        return true;
    }
