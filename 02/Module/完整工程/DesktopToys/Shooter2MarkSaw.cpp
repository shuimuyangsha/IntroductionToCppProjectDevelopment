...
CShooter2MarkSaw::CShooter2MarkSaw(float x, float y)
{
    SetCenterPos(x, y);
    ...
    _stprintf_s(szFilename, _T("res/The Toolbox Mark Saw %02d.png"), i);
    ...
    for(int i = 0; i < (1 + rand() % 3); ++i) {
        SDynamic dy;
        // 下落物体
        dy.m_img2 = Image::FromFile(_T("res/The Toolbox Mark Saw 06.png"));
...
CShooter2MarkSaw::~CShooter2MarkSaw()
{
}
...
void CShooter2MarkSaw::Draw(Gdiplus::Graphics &gh)
{
    // 主图片
...
bool CShooter2MarkSaw::IsChanging() const
{
    // 没有空，说明还有动态对象
    return !m_vD.empty();
}
